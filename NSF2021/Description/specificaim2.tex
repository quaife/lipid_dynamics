...
\begin{align*}
  u^{N+1} = u^{N} - \alpha (dF^N)^{-1} F(u^N),
\end{align*}
where $dF^N = -\rho^2 \Delta + f''(u^N)$ is the Fr\'{e}chet derivative
...
\begin{wrapfigure}[]{l}{1.8in}
  ...
  \centering
  \input{figures/doublewell.tikz} 
  \caption{\label{fig:CA} (a) The double well potential $U_1 =
  0.5$, $U_2 = 2$, $a = 1$, $b = 0.4$. (b) The solution of the
...
$dF^{-1}$ is defined in terms of the piecewise quadratic approximation
\begin{align*}
...
to~\eqref{eq:F} by considering a finite difference method applied to its
one-dimensional version in $(0,1)$ with the Dirichlet boundary
conditions $u(0) = U_0$ and $u(1) = U_1$.  These parameters result in
the double well potential illustrated in Figure~\ref{fig:CA}(a), and the
inexact Newton's method converges to the solution in
Figure~\ref{fig:CA}(b). Figure~\ref{fig:CA}(c) shows the max norm of the
residual at each Newton iterate when using the exact second derivative
(red curve) and the piecewise constant approximate (blue curve).
