...
backtracking.
As expected, the biggest challenge amounts to solving
\begin{align}
  \label{eq:NewtonUpdate}
  - \rho^2 \Delta v + f''(u^{N}) v = F[u^N].
\end{align}
\begin{wrapfigure}[20]{r}{1.8in}
  \centering
  \input{figures/doublewell.tikz}  \\
  \input{figures/residuals.tikz}
  \caption{\label{fig:residuals} A double well (top) and the error (bottom)
    for the case $U_1 = 0.5$, $U_2 = 2$, $a = 1$, $b = 0.4$.}
\end{wrapfigure}
