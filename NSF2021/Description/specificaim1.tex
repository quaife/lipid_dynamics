...
molecules. The free energy functional take the form 
\begin{align*}
...
The density~\eqref{eq:stress} was first
...
and scaling to case (a), which amounts to changing one line of code. 
...
\begin{wrapfigure}[11]{r}{0.65\textwidth}
  ...
  disk and the arrow is its director.}
\end{wrapfigure}
...
gives a particle with a hydrophobic intensity three times as great
...
\begin{figure}[h!]
  \begin{center}
    \begin{tabular}{m{0.1cm}m{1.9in}m{1.9in}m{1.9in}}            
      (a)
    &\includegraphics[width=1.9in]{figures/SpecificAim1/N100B1.pdf}
    &\includegraphics[width=1.9in]{figures/SpecificAim1/N100B2.pdf}
     &\includegraphics[width=1.9in]{figures/SpecificAim1/N100B3.pdf}    \\
     (b)
    &\includegraphics[width=1.9in]{figures/SpecificAim1/N100C1.pdf}
    &\includegraphics[width=1.9in]{figures/SpecificAim1/N100C2.pdf}
      &\includegraphics[width=1.9in]{figures/SpecificAim1/N100C3.pdf}    \\
    (c)
      &\includegraphics[width=1.9in]{figures/SpecificAim1/N100A1.pdf}
      &\includegraphics[width=1.9in]{figures/SpecificAim1/N100A2.pdf}
      &\includegraphics[width=1.9in]{figures/SpecificAim1/N100A3.pdf}    \\
      &\multicolumn{1}{c}{$t = 0$}
      &\multicolumn{1}{c}{intermediate time}
      &\multicolumn{1}{c}{$t = t_{\text{final}}$}
  \end{tabular}
    \end{center}
  \caption{\label{fig:self-assembly}
    Using the same initial configuration,
    particles self-assemble into distinct morphologies based
    on the boundart condition $g.$ 
    boundary condition leads to a checkerboard pattern.
    In row (a), self-assembly with an amphiphilic boundary
    condition \eqref{eq:bcs}(a) results a bilayer pattern shielding
...
\todo[inline]{Rationalize and citations for following} 
Finally, boundary condition \eqref{eq:bcs}(c) models 
a particle with oppositelly charged ends making the 
...
the syntheses soft matter materials.
...
because the model parameters control particle surface properties independently, 
whereas these properties are often difficult to decouple in experiment
~\cite{Bradley2016,Mallory2017,Bradley2017}.

One challenge is translating phenomenological parameters
into measurable quantities.  For example, Janus particles
with uniform surface properties are costly or difficult to
manufacture.  Thus, we can replace the ideal boundary
...
The square of this function behaves as a normal distribution with variance
like $1/\kappa_i$. We can replicated surface imperfections
by randomizing the concentration $\kappa_i$ and particle radii $r_i$,
for example. Collaboration with experimental groups will
...
A natural generalization of the linear response potential $f(u) = u^2$
is a fourth-order, tilted double-well potential
...
and $u_1$ gives a local minimum for the
...
PI RR and colaborators launched the
...
The key innovation is that when particles are
...
Gompper introduced the double-well model for one-dimensional hydrophobic
attraction \cite{GoHaKo94}.
The results improved on previous work~\cite{ErLjCl89,MaRa76},
but the realistic hydrophobic effect is more
complicated and may involve higher-dimensional geometric features. 
Some of the model parameters directly
...
This improved model will allow us to more accurately 
simulate colloidal suspensions.
...
\todo[inline]{Below needs citation and more solid explanation}
Other approaches will be required if the modified phenomenological
theory does not recreate the measured force curves.
