...
Finally, in terms of mathematical justifications, we mention
...
continuous surface or curve $\Sigma$.
...
The variation of \eqref{eq:Canham-Helfrich} gives rise to a surface force
that pointwise balances the jump in hydrodynamic stress across the interace.
A vesicle is closed surface, and so while energy does involve
the Gaussian curvature, it does not enter \eqref{eq:Canham-Helfrich}
since Gauss-Bonnet Theorem guarantees that
the integral of Gaussian curvature is independent of the shape of $\Sigma$.
...
form $k_A(\alpha - \alpha_0)^2/\alpha_0$ where $\alpha$ and $\alpha_0$ are the
...
\cite{}.
These kinds of topological changes are clumsy to incorporate into 
...
Diffusive interface models and surface-director models partially make up for some of
these shortcomings. 
...
The question lipid orientations is closely related to the vast and well-studied
field of nematic liquid crystals.  Here there has been recent development
in the simulation and theory of nematic liquid crystals on surfaces.
...
large forces will pull the granules apart as physically required.
(ii) The model formulation is built around important details like lipid orientations,
and exhibits and inter-monolayer slip not present in diffusive interface models.
...
For small granule numbers, the bilayers form a single stable, ring-shaped vesicle
...
$E(t)$ and $E_{\min}$ is the elastic energy stored by granules.
Indeed, if a background flow overcomes the energy barrier to force granules apart,
then bilayer unfolds from its circular shape into a flat shape.  

When many granules are involved, the granules reach an intermediate
