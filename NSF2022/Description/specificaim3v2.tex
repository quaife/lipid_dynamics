...
PIs' recent analyses in \cite{fu-ryh-qua-you2022} show that these patterns may be understood by a mixture of polar and nematic characteristics of the individual Janus particle, 
tuned by the boundary condition $g({\bf x})$, and lead to the following {\bf Problem:} Can the collective dynamics of Janus particles with different boundary conditions 
...
PI Young proposes to construct a kinetic theory to quantify the underlying mechanisms that lead to distinct morphologies of Janus particles described above.
Consider the suspension dynamics on a length scale much longer than the particle size, and denote the number density of 
Janus particles at time $t$ with position ${\bf x}$ and director vector ${\bf p}$ ($|\bf p|=1$) as $\psi({\bf x}, {\bf p},t)$, which satisfies the 
...
Within this framework, the stress on the viscous solvent from the Janus particles can be computed
...
dynamics and rheology of suspension of Janus particles controlled by tuning the boundary conditions on the microscopic Janus particles, which the PIs recently investigated using direct numerical simulations and the results are summarized in \cite{fu-ryh-qua-you2022}. Working together with PIs Quaife and Ryham, the team proposes to extract the coefficients
$K$ and $K_P$ for a single Janus particle first from the direct numerical simulation results. Using these values, the dynamics, pattern formation and rheology
predicted from analyzing the proposed coarse-grained model will be compared quantitatively against numerical simulations. 

It is possible that the outcome of such quantitative comparison may depend on how collisions between Janus particles are treated in the simulations (see \S~\ref{sec:specificaim2}), 
and how the steric interactions are 
incorporated in the kinetic theory. If this turns out to be an important effect, PI Young will incorporate the formulation of constraint forces as part of the active stress in the Stokes equations for the suspension.  Such constraint forces arise from the geometric 
constraint that the Janus particles cannot overlap with each other \cite{Weady2022_PRF}. 
