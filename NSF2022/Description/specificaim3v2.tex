...
amphiphilic granuels at time $t$ with position ${\bf x}$ and director vector ${\bf d}$ ($|\bf d|=1$) as $\psi({\bf x}, {\bf d},t)$, which satisfies the 
...
$K$ and $K_d$ for a single granuel first from the analysis (\S~\ref{sec:specificaim1}) and the direct numerical simulation results (\S~\ref{sec:specificaim2}). 
