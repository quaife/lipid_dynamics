...
the HAP model by shifting the boundary condition $g$ \cite{FuEtAl2022_arXiv}.
...
\int d{\bf x} \left\{\frac{K_p}{2} \left(\nabla{\bf p}\right)^2 + \frac{K}{2}\left(\nabla\left({\bf p}{\bf p}^T- \frac{\bf I}{2}\right)\right)^2\right\},
\end{align}
where the subscripts denote the variables the gradient operator is taken with respect to, 
...
the phoretic Janus particle suspension \cite{TraversoMichellin2020_PRF}.
...
$K=0$ when the suspension is purely polar, and $K_p=0$ when the suspension is purely nematic \cite{Amiri2022_JPhysA}. 
%
...
nematic fluids \cite{YoungShelleyStein2021_MBE}. Within this framework, the stress on the viscous solvent from the active particles can be computed
by integrating over the whole domain the stress from the single particle weighted by the density distribution $\psi$ \cite{TraversoMichelin2020_PRF}.
In particular, PI Young proposes to use the Bingham closure approximation \cite{YoungShelleyStein2021_MBE} to approximate these calculations to study the
dynamics and rheology of suspension of Janus particles controlled by tuning the boundary conditions on the microscopic Janus particles, which the PIs recently investigated using direct numerical simulations and the results are summarized in \cite{Fu2022_arXiv}. Working together with PIs Quaife and Ryham, the team proposes to extract the coefficients
