...
The PIs investigated how distinct morphologies can be obtained from the
...
on $W(\phi) = \phi^2$, equation~\eqref{eqn:phase} is the
...
\begin{equation}
  ...
  \begin{tabular}{c|c|c}
    ...
    \hline
    \rule{0pt}{4ex} 
      $h(\mathbf{x}) = \displaystyle \frac{1+\cos (\theta_i(\xx))}{\sqrt{3\pi r}}$
    & $h(\mathbf{x}) = \displaystyle \frac{2+\cos (\theta_i(\xx))}{\sqrt{9\pi r}}$
    & $h(\mathbf{x}) = \displaystyle \frac{\cos (\theta_i(\xx))}{\sqrt{\pi r}}$
\end{tabular}
\end{equation}
...
\ref{fig:flow_map}). The normalizations (e.g., $(\pi r)^{-1/2}$) provide
a uniform surface energy $\int_{\Gamma_i} h^2 \,ds = 1$ for circular
particles of radius $r$. The cases (b) and (c) are obtained by applying
...
Multilamelar bilayers arise when both sides of the granule are
