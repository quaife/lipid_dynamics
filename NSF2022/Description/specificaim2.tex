...
$\phi_0$ and $\phi_1$, we will interpret the solution
...
using backtracking. As expected, the biggest computational challenge is
solving $-\epsilon^2 \Delta v + \tfrac{1}{2}W''(\phi^{N}) v =
J[\phi^N]$. Solving this variable coefficient PDE with an integral
equation is challenging since a fundamental solution is not readily
available. However, by choosing a double well potential with
...
as $v = \phi^P + \phi^H$ where $\phi^P$ is a volume potential and
$\phi^H$ is a layer potential. The volume potential is
\begin{align*}
  \phi^{P}(\xx) = \int_{\Omega(t)} G(\xx,\yy) J[\phi^N](\yy)\, d\yy, 
    \quad \xx \in \Omega(t),
\end{align*}
where $G$ is the fundamental solution of $-\epsilon^2 \Delta +
\tfrac{\beta}{2}$. Volume potentials are more challenging to compute
numerically than layer potentials. However, very recently, a suite of
new methods have been developed to accurately and efficiently compute
volume potentials~\cite{fry-leh-tor2018, and-zhu-vee2022}. The boundary
