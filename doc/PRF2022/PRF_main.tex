...
in a viscous background flow (JFM, 941, 2022). We investigate how various bilayer structures arise from tuning the hydrophobicity/hydrophilicity of the JPs. 
...
%This means
%that the total force $\FF_i,$ and torque $T_i$ from the hydrodynamic
%stress on each particle (with subscript `$i$') balance the force and
%torque from the hydrophobic potential~\cite{Fu2022_JFM}. 
The total force
...
%particles and the viscous solvent. 
...
In the followings we let $c$ be the radius of each JP.
...
\end{alignat}
where the boundary condition $g$ gives the degree of hydrophobicity the
particle-solvent interface.

The hydrophobic stress is
\begin{align}
\mathbf{T}
= \gamma \left[ \rho^{-1} u^2 \mathbf{I}
  + \rho \left(|\nabla u|^2 \mathbf{I} - 2\nabla u \nabla u^T\right)\right]
\end{align}
and is derived by applying variation of domain techniques to the free
energy~\eqref{eq:free_energy}~\cite{Fu20}. Then, the hydrophobic force
and torque are
\begin{align}
...
\end{align}
which balance the hydrodynamic force and torque~\eqref{eq:force}, for $i
= 1,\dots,N_b$. 
%The term 
%\begin{align}
%\mathbf{T}
...
%  + \rho \left(|\nabla u|^2 \mathbf{I} - 2\nabla u \nabla u^T\right)\right]
%\end{align}
%is the hydrophobic stress~\cite{Fu20} and derived by variation of the
%domain techniques.

Finally, the dimensionless repulsion profile $P$ takes the form $P(s) =
1 - \sin(\pi s/2)$ for $0 \leq s < 1$ and $P(s) = 0$ for $1 \leq s <
...
We use the same parameters from previous work~\cite{Fu2022_JFM}: $\mu =
1$ cP, $\gamma = 4$ pN nm$^{-1}$, $\rho = 5$ nm, $c = 1.25$ nm, $M = 2$
pN, and $\rho_0 = 0.5$ nm.
...
With max$(g)>0$ to guarantee (at least partial) hydrophobic surface on each JP,  
the boundary condition can be classified into three categories: min$(g) >0$, min$(g) =0$, and min$(g) <0$. Without loss of generality, 
...
respectively. 
Case (i) with $b = 1$ (bc(i)) mimicks an amphiphilic particle.
...
In case (ii) with $b = 1$ (bc(ii)) the boundary condition is everywhere positive.
...
To begin, we place the bilayer structure (with bc(i)) from the inset of Figure~\ref{fig:relax}(b)
...
shows snapshots of the bilayer when $\dot\gamma= 0.05,0.075,0.01$
at $t = 0.5$ $\upmu$s.
The dynamics of the bilayer can be found in the supplementary movies, which show that the initial pieces of bilayers merge and form a worm-like shape
under a linear shear flow.
At a low shear rate, the bilayer constantly goes through rotation and extensional deformation with the whole bilayer intact (figure~\ref{fig:BC1_shear}(a)).
As the shear rate increases to a moderate value (figure~\ref{fig:BC1_shear}(b)), the bilayer is observed to shed a small piece of bilayer, 
...
In terms of deformation measures,  
the free energies 
are relatively constant (Figure~\ref{fig:BC1_shear}(d)),
with the overall changes in the range of a few kT/nm.  
Note that the free energy $F$ in figure~\ref{fig:BC1_shear}(d) has the unit of force because our system is two dimensional.

An interesting feature of the simulations is that the order parameter $\tilde S_2$ in figure~\ref{fig:BC1_shear}(e)
...
result in an overall decrease in distinct bilayer structures.  
This suggests that
...
\end{figure}
%
In \citet{Fu2022_JFM} the hydrodynamics of a JP vesicle under a linear shear flow is studied. 
Focusing on drawing analogies to hydrodynamics of a vesicle in continuum modeling (such as tank-treading, bilayer slip and permeability),
our previous studies also show the existence of a critical shear rate above which the JP vesicle ruptures. 
Based on the these early results, we conduct numerical simulations with $\dot\gamma=0.05, 0.066, 0.075$.
At these shear rates, the vesicle undergoes the familiar hydrodynamics (such as elongation along the extensional axis and tank-treading motion),
as shown in Figure~\ref{fig:Ves_shear} (a)--(c).
%
Here we illustrate how the three measures of structural deformation 
...
Unlike the dynamics of bilayers in figure~\ref{fig:BC1_shear}, 
the energies of the JP vesicle in figure~\ref{fig:Ves_shear}(d) show jumps from the baseline value $30$ kT/nm to 32 kT/nm
signaling the rupture of the vesicle into disconnected bilayers
(see figure \ref{fig:Ves_shear}(d), long and short dashed curves).
...
(see figure \ref{fig:Ves_shear}(e), solid curve).
...
(see figure \ref{fig:Ves_shear}(e), dashed curves).
...
%
As in the previous case, peaks in the strain parameter $E$ correlate well with changes in structural topology in the bilayer dynamics, see
the dashed curves in figure~\ref{fig:Ves_shear}(f).
...
\left(
-\cos\left(\frac{{\bf e}_x\cdot\xx}{l}\right)
 \sin\left(\frac{{\bf e}_y\cdot\xx}{l}\right)
         {\bf e}_x
         +\sin\left(\frac{{\bf e}_x\cdot\xx}{l}\right)
         \cos\left(\frac{{\bf e}_y\cdot\xx}{l}\right)
             {\bf e}_y\right).
\end{equation}
...
the dimensionless flow strength $V_0$ and with the cell size $l$ nm.
For most of the results here the cell size $l=2$ is used in this subsection.
%
