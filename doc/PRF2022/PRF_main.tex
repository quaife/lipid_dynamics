...
\includegraphics[width=1.0\textwidth]{Figures/Figure2.pdf}
  \end{center}
  \vspace{-20pt}
  \todo[inline]{We should capitalize bc is that it reads BC like we have
  in the text.}
...
A single vesicle in a shear flow. Panels (a)--(c) are snapshots for
  ...
  streamlines are plotted in the background.  Panel (d) plots the free
...
\begin{figure}
  ...
  The streamlines are plotted in the background. Panel (d) plots the
...
In these cases, there is little change in the vesicle free energy and
order (Figure~\ref{fig:ves_TG}(d)-(f))).
...
The relative length $L/L_0$, however, is constant in $t$ for all cell
sizes (Figure~\ref{fig:BTG_Scale}(e), inset).
...
of Figure~\ref{fig:BC2_shear}(a))
in shear flows with $\dot\gamma=0.05, 0.1, 0.15$ and in TG flow with flow rates $V_0=0.1, 0.15, 0.2$.
For the striated case, we also place a 60-particle configuration (inset
of Figure~\ref{fig:BC3_shear}(a))
...
(Figure~\ref{fig:BC3_shear} and Figure~\ref{fig:BC3_TG}).
Conversely, a $25 - 50 \%$
...
with shear rate (Figure~\ref{fig:BC2_shear}(f)).
...
to deform the body (Figure~\ref{fig:BC3_shear}(f)).
...
under low shear rates (Figure~\ref{fig:BC1_shear}(d,f)).
...
observed in the TG background flow case (Figure~\ref{fig:BC1_TG}).
...
equilibrium (Figure~\ref{fig:relax}(d), top, middle rectangle)
