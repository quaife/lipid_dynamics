...
A suspension of $N_b$ Janus particles is immersed in a vicsous solvent.
The domain $\Omega(t) \subset \mathbb{R}^2$ denotes the solvent phase
where $t$ is time.
The particles are disks of radius $c$ with center $\aa_i$ and
orientation $\theta_i$ relative to a fixed horizontal axis.  
...
background flow, and $\mu$ is the constant viscosity.
On the boundaries, the solvent
velocity satisfies a no-slip boundary condition for a rigid body motion
\begin{equation}
  \label{eq:rigid-vel}
...
To get $\FF_i$ and $T_i$,
the particles minimizes the free energy
\begin{equation}
...
in the presence of hydrophobic surfaces,
...
\end{gather}
The boundary condition $g$ represents the degree of hydrophobicity
...
$
is the hydrophobic stress it is derived from 
variation of the domain as described in ~\cite{Fu2018_SIAM}.
...
a $\rho_0$-tubular neighborhood of $\Gamma_i$
contribute to repulsion.  The parameter $M$ is the repulsion modulus.
...
and where $\sigma$ is a scalar-valued density function.
...
Once determined the translational and angular velocities $\vv_i, \omega_i$
...
\begin{figure}
\begin{center}
  \includegraphics[width=0.24\textwidth]{Fig2a.png}
  \includegraphics[width=0.24\textwidth]{Fig2b.png}
  \includegraphics[width=0.24\textwidth]{Fig2c.png}
  \includegraphics[width=0.24\textwidth]{Fig2d.png}
\end{center}
\begin{caption}{\label{fig:relax}
  Panel (a) shows 60 particles confined to a square with random 
  positions and orientation.
  In a quiescent background flow, 
  the self-assembled particles take on distinct configurations
  depending on the value of the shift parameter $b$.}
\end{caption}
\end{figure}
...
We refer to the side of the particle where $\alpha = 0$ as the head and the
side where $\alpha = \pi$ as the tail. 
The boundary conditions are
normalized so that $\int_{\Gamma_i} g^2(\xx) \, \dif s = 1$.

To see the effect of the shift parameter $b$, we simulate the dynamics of
60 particles in a quiescent flow as shown in Figure~\ref{fig:relax}.
There are three cases:
(i) $b = 1$, bilayers,
(ii) $b = 2$, multilamellar
and (iii) $b = 0$, striated.

Case (i), $b = 1$, mimicks an amphiphilic particle.
...
In case (ii), $b = 1$ the boundary condition is positive everywhere.
It takes the maximum value $3a$ on head where $\alpha = 0$ and the
minimum value $a$ on the tail side where $\alpha = \pi$. This way,
...
multilamellar structure with 2 layers. The number of folds depends on several factors,
for instances, number of particles and intial configurations.
...
alternating orientations \cite{}. Unlike the electrical case, though,
the chains form striated layers
...
the Euclidean distance matrix $D(t)$,
...
\end{equation}
%
This is simpler because the boundary data $g$ are known.
...
The $N_b$ by $N_b$ Euclidian distance matrix is 
\begin{equation}
  \label{eq:EDM}
[D(t)]_{ij} = \|\aa_{i} - \aa_{j}\|, \quad i, j = 1, \dots, N_b.
\end{equation}
...
particles behaves as a rigid body i.e., $\|D(t) - D(0)\| \ll 1$. 

To study the change in orientational order, we use the scalar order parameter 
\begin{equation}
  \label{eq:S2}
S_2(t) = \frac{1}{N_b} \sum_{i=1}^{N_b} \frac{1}{2}(3\cos^2(\theta_i - \bar \theta) - 1).
\end{equation}
Here $\bar \theta$ is a circular mean defined as the
orientation of the largest eigenvalue eigenvector of the projection 
$\dd_1\dd_1^\top + \dots + \dd_{N_b}\dd_{N_b}^\top$.
A value $S_2(t) = 1$ indicates a complete orientational order where all particle
directors lie on a common axis. A value far from 1 indicated disorder.

It turns out that $S_2$ is an inadequate measure of order for the bilayer and
multilamellar configurations.  For example, the directors of a circular
...
\begin{figure}
  \begin{center}
  \includegraphics[height=0.2\textheight]{multilamellar_shear_Energy}
    \includegraphics[height=0.2\textheight]{multilamellar_shear_MSD}\\
          \includegraphics[height=0.2\textheight]{multilamellar_shear_LocPar}
  \end{center}
...
\begin{figure}
  \begin{center}
  \includegraphics[height=0.2\textheight]{multilamellar_TG_Energy}
    \includegraphics[height=0.2\textheight]{multilamellar_TG_MSD}\\
    \includegraphics[height=0.2\textheight]{multilamellar_TG_LocPar}
  \end{center}
...
\begin{figure}
  \begin{center}
  \includegraphics[height=0.2\textheight]{checker_board_shear_Energy}
    \includegraphics[height=0.2\textheight]{checker_board_shear_MSD}\\
  \includegraphics[height=0.2\textheight]{checker_board_shear_LocPar}
  \end{center}
\caption{A checker board in shear flows with shear rates $\dot\gamma=\{0.125, 0.15\}$. We propose the order parameter defined by $S_2 = \frac12(3\cos(\theta-\bar\theta)^2-1)$}
\end{figure}


\begin{figure}
  \begin{center}
  \includegraphics[height=0.2\textheight]{checker_board_TG_Energy}
    \includegraphics[height=0.2\textheight]{checker_board_TG_MSD}\\
      \includegraphics[height=0.2\textheight]{checker_board_TG_LocPar}
  \end{center}
