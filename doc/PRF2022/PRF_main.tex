%% ***A*** Start of file apstemplate.tex ****** %
%%
%%
...
\end{center}
\caption{\label{fig:s2esummary}asdf}
\end{figure}
...
simulation \cite{Bradner}, those for large unilamellar vesicles (LUV),
\cite{Bernard2005ShearinducedPA} and shearing in blood capillary vessels 
 where shear rates range over 100--1000 s$^{-1}$ \cite{NEEDED}.
...
to a rounded diamond. \hl{Surprisingly, the direction of rotation switches,
so that for $\lambda = 1$ nm, the rotation is counterclockwise while
for $\lambda = 3$ nm, the rotation is clockwise, for example.  }
...
To estimate $h(\kk)$, 
the $N_b = 60$ JP
vesicle in Figure \ref{fig:ES2_summary}(e) 
...
and five, respectively, than for Type (I) (Figure \ref{fig:F_summary}(c)), making thermal fluctuation
even more negligible in these cases.
