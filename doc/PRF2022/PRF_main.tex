...
\subsection{Energy Calculation via Singularity Subtraction}

Suppose we split the solution to the screened Laplace boundary value problem~\eqref{eq:stokes}
into smooth portion $w_i$ and singular portion $u_i$. That is, $u = w_i + u_i$.
...
= C\rho\sum_{i=1}^{N_b}\int_{\Gamma_i} g \left(\frac{\partial w_i}{\partial \nnu}+\frac{\partial u_i}{\partial \nnu} \right) \, \dif s, \quad \xx\in\Gamma_i.
\end{align}
%%%
...
\label{eq:free_energy3}
  E[u(\xx)] &= C\rho\sum_{i=1}^{N_b}\left(\int_{\Gamma_i} g \frac{\partial w_i}{\partial \nnu} \,\dif s 
  + \int_{U_i}\nabla g_i \nabla u_i + g_i (u_i+\frac12\sigma) \,\dif \xx\right)\\
  &= C\rho\sum_{i=1}^{N_b}\left(\int_{\Gamma_i} g_i \frac{\partial w_i}{\partial \nnu} 
 + u_i \frac{\partial g_i}{\partial \nnu} \,\dif s - \int_{U_i}\Delta g_i u_i - g_i (u_i+\frac12\sigma) \,\dif \xx\right) \\
 &= C\rho\sum_{i=1}^{N_b}\int_{\Gamma_i} g \frac{\partial w_i}{\partial \nnu}+ (u_i+\frac12\sigma)  \frac{\partial g_i}{\partial \nnu}\,\dif s, \quad \xx\in\Gamma_i.
\end{align}
%
