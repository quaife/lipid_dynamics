...
+\int_{\Gamma_i}\left(u_i+\frac12\sigma_i\right)\frac{\partial u}{\partial \nnu} \,\dif s     
- \int_{\mathbb{R}^n\setminus \overline{U}_i} u_i\Delta u\,\dif \xx \right)\\
&= C\rho\sum_{i=1}^{N_b}\left(\int_{\Gamma_i} u \frac{\partial w_i}{\partial \nnu} + 
\left(g_i+\frac12\sigma_i\right)\frac{\partial u}{\partial \nnu}   \,\dif s \right), \quad \xx \in \Gamma_i.
\end{align}
...
Similar to shear flow simulations, we then place equilibrium states of all boundary conditions \eqref{eq:bcs} in a rotating Taylor-Green Flow
\begin{equation}
\uu_\infty(\xx) = V_0 \left(-\cos({\bf e}_x\cdot\xx)\sin({\bf e}_y\cdot\xx)){\bf e}_x+\sin({\bf e}_x\cdot\xx)\cos({\bf e}_y\cdot\xx)\right){\bf e}_y.
\end{equation}
%
