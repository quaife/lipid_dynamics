...
Unbouned Flow: 
\begin{enumerate}
\item $N_b=60$ self-assembly/relaxation runs with all 3 boundary conditions using $N=24$ points per particle (FIG. 2). Energy profiles in a linear-log plot (FIG. 3) ($E(t)$ or $\Delta E(t)$ v.s. time).

\item $N_b=198$ relaxation runs with all 3 boundary conditions using $N=24$ points per particle.
Add energy profiles to check local/absolute minimum.

\item $N_b=60$ structures in a shear flow: For both boundary conditions, lamellar (FIG. 4) and polar (FIG. 4), check convergence using $N=16$ and $N=24$. 
Compare the new results with the current FIG 4. (d). Revise FIG. 5 with more details. 

\item Use particle/checker board alignment to identify the critical shear rate for phase separation.
...
Channel Flow: 
\begin{enumerate}
\item $N_b=25$ vesicle in a narrow channel.
\item $N_b=40$ multi-lamellar structure in a narrow channel.
\end{enumerate}
...
%
Notice that for this simulation, the center of mass position of the initial configuration is not at the origin and it is slightly to the right of the origin. Therefore, the result has shown that the structure stretching occurs earlier than the case when we shift the center of mass position to the origin. 
%
\begin{figure}
  \begin{center}
