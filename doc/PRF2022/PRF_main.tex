...
boundary integral numerical simulations \cite{fu-qua-ryh-you}. Janus particles consist of both a hydrophobic and a hydrophilic surfaces. 
...
Under background flows, the hydrodynamics of a JP vesicle (a self-enclosed bilayer of JPs) exhibit many familiar behaviors of a vesicle \cite{fu-qua-ryh-you}: 
...
\includegraphics[width=1.0\textwidth]{Figures/Figure2.pdf}
  ...
  A:  The free energy \eqref{eq:free_energy} decreases
    ...
    particles. We use different colors to distinguish from boundary conditions.
    B: The local order parameter $\tilde{S}_2$ for all 6 relaxation runs. 
...
Figure~\ref{fig:relax_energy}a tracks the free energy profiles for all relaxation runs. For 198-particle cases, we normalized the energies by 60/198 and the results show that all energies decrease with good agreements in all boundary conditions. This can be considered as an evidence that the proposed model with specified boudary condtions is independent of number of particles.
Figure~\ref{fig:relax_energy}b 
measures the orientational order parameter $\tilde{S}_2$. Both bilayer and multilamellar 
structures have similar trends in small and large systems. The aggregation for striated configurations may have one or more different pattern formations when the number of particles is varied. 
...
\begin{figure}
  ...
  \end{center}
  \caption{
    \label{fig:BC1_shear} Multiple component bilayers in a shear flow when $\dot \gamma = \{0.05, 0.075, 0.1\}$ at $t=0.5\ \mu$s. The pre-relaxed initial configuration is shown in inset of panel (a). The streamlines are plotted in the background. Panel (d) plots the free energies; panel (e) shows orientational parameter $\tilde{S}_2$; panel (f) shows positional parameter E.
}
\end{figure}
...
\begin{figure}
  ...
  \caption{
    \label{fig:Ves_shear}
Single vesicle in a shear flow. Panels (a)-(c) are snapshots for $\dot \gamma = \{0.05, 0.0655, 0.075\}$ at $t=0.16\ \mu$s where the pre-relaxed initial configuration is shown in inset of panel (a).
...
Our previous paper \cite{fu-qua-ryh-you} studied the geometry,
...
\begin{figure}
  ...
  \end{center}
\caption{\label{fig:ves_TG} Single vesicle in a shear flow. Panels (a)-(c) are snapshots for $V_0=\{0.1, 0.15, 0.2\}$ at $t=0.6\ \mu$s where the pre-relaxed initial configuration is shown in inset of panel (a). The streamlines are plotted in the background.
Panel (d) plots the free energies; panel (e) shows orientational parameter $\tilde{S}_2$; panel (f) shows positional parameter E.
}
\end{figure}
