...
\begin{caption}{\label{fig:relax}
  ...
  positions and orientation and the inset shows $N_b=60$ particles configuration.
 ...
 Panel (b)--(d) insets are configurations using $N_b=60$ sharing with the same color map.}
\end{caption}
\end{figure}
...
state for 60-particle run that forms multiple bilayers with a vesicle like structure.
...
multilamellar structure with multiple layers.
At equilibrium, the number of folds depends primarily on the number of particles.
In addition, the inset provides a 3-fold structure when there are 60 particles.
...
grid and the orientations alternate between layers (Figure~\ref{fig:relax}(d)). We observe that 
when the number of particles is fewer, there is only single alternating pattern in particle orientations. Larger number of particles results two different patterns as shown in the figure.
...
To measure positional order, we use a strain parameter 
\begin{equation}
\label{eq:SP}
E(t) = \frac{1}{N_b} \sum_{i=1}^{N_b} \|E_i\|
\end{equation}
where $E_i = \tfrac{1}{2}(F_i^TF_i - I)$ is a Green-Lagrange strain tensor and $F_i$
is an approximate deformation gradient at particle $i$; $\| \cdot \|$ is the Frobeneous norm.
The motivation for  \eqref{eq:SP} is that 
for relatively weak background flow strengths, the near-equilibrium particle
collections behave as a solid and $E(t)$ is close to zero.
More generally, the Green-Lagrange strain tensor $E_i$ 
can be used to encode the energy of elastic deformations from the reference (equilibrium)
to deformed configuration and $E(t)$ gives a sense of how significant these deformations
may be.  Lacking a map from the reference state, 
we approximate a deformation gradient $F_i$ using weighted least squares for the system 
\[
F_i(\aa_j(t) - \aa_i(t)) = \aa_j(0) - \aa_i(0),\quad j = 1,\dots, N_b  
\]
with weights $w_i = \exp(-\|\aa_j(t) - \aa_i(t)\|/4c)$ where $c$ is particle radius. 
...
\includegraphics[width=1.0\textwidth]{Figures/Figure2.pdf}
  ...
  A:  The free energy \eqref{eq:free_energy} decreases
    ...
    The black curves are the energies for the $N_b=60$ cases and  
    ...
    particles. B: The local order parameter $\tilde{S}_2$ for all 6 relaxation runs. 
    The curve styles are identical to the ones in panel A.
    The near-equilibrium configurations depend on the initial configuration and the results show that for both $N_b=60$ and $N_b=198$ cases, bilayer and multi-lamellar configurations have similar trend in self-assembly in term of the local order parameter. The results of the striated configuration depend on the initial setup.
  }
\end{figure}


Figure~\ref{fig:relax_energy}a tracks the free energy profiles for all relaxation runs. For 198-particle cases, we normalized the energies by 60/198 and the results show that all energies decrease with good agreements in all boundary conditions. Figure~\ref{fig:relax_energy}b 
measures the orientational order parameter $\tilde{S}_2$. Both bilayer and multilamellar 
structures have similar trends in small and large systems. The aggregation for striated configurations may have a significant difference when the number of particles is varied.
...
\begin{figure}
  ...
  \caption{
    ...
    and the multiple component bilayers in shear flow (The insets of Fig~\ref{fig:relax}b is used for panels (e); panels (f)-(h): $\dot \gamma = \{0.05, 0.075, 0.1\}$ at $t=500$). The streamlines are plotted in the background. }
\end{figure}

The bilayer is subject to a shear background flow
\begin{equation}
...
The free energies (Figure~\ref{fig:BC1_shear}d) are relatively constant and 
lower shear rate results a smaller orientational parameter (Figure~\ref{fig:BC1_shear}e). The supplementary movies show that these bilayers merge and form a work-like shape in higher 
shear flows. Figure~\ref{fig:BC1_shear}f shows that the strain parameter diverges when 
the shear rate is large enough.
...
In Figure~\ref{fig:Ves_shear} (a)--(c), the vesicle in a shear flow undergoes the familiar tank-treading deformation. 
Our previous paper \cite{Fu2022_JFM} studied the geometry,
intermonolayer slip, and critical shear rates $\gamma^*=0.066$
for vesicles comprised of amphiphilic Janus particles. 
Here $\dot\gamma=0.05, 0.066, 0.075$ and there are jumps in energies (Figure \ref{fig:Ves_shear}d, long and short dashed curves) due to the breaking of the vesicle into two
components. There is greater variation in the order parameter $\tilde{S}_2(t)$ as well
with a baseline value around $0.7$
(Figure \ref{fig:Ves_shear}e, dashed curve).
The oscillations occur due to the orbit of the two bilayer components, and $\tilde{S}_2(t)$ increases to about $0.9$ when the components are nearly parallel and slide past one another.
For the lowest shear rate case, it has nearly perfect orientational parameter due to the steady tank-treading motion.
Positional order, however, diverges since the two component configuration
is uncorrelated to the initial circular configuration (Figure \ref{fig:Ves_shear}f, dashed curve).
...
\end{equation}
where $l$ sets the size of the rotating cells and $V_0$ is the flow strength. The default parameter
$l=2$ is used through this work.
%
...
\begin{figure}
  ...
  \end{center}
  \vspace{-20pt}  
  \caption{\label{fig:BC1_TG} The figure shows a vesicle in Taylor-Green flow (panel (a) shows the pre-relaxed initial configuration; panels (b)-(d): $V_0 = \{0.1, 0.15, 0.2\}$ at $t=600$) 
...
Figure~\ref{fig:BC1_TG}a--c are the snapshots when $V_0=0.1,0.15,0.2$, respectively.
This initial configuration in TG flow encounters a number of fusion but there is no clear trend for
all measures with respect to the flow strengths.
...
\includegraphics[width=1.0\textwidth]{Figures/Figure7.pdf}
  \end{center}
  \vspace{-20pt}  
  \caption{\label{fig:BTG_Scale} With $V_0=0.1$, panels (a)-(d) show the configurations with streamlines when a vesicle is in Taylor-Green flow at $t=1000$ and $l= 1,2,3,4$, respectively.
  Vesicle in a Taylor-Green flow where $V_0=0.1$ and the size of rotating cell $l$ is varied. The enclosed area calculated from the bilayer midplane decreases over time and the total length measured from the midplane remains nearly constant with small magnitude of oscillations.
  }
\end{figure}


We vary the cell size by tuning the parameter $l$ in \eqref{eq:tg_flow}.  Figure~\ref{fig:BTG_Scale}a--d show the deformation of a vesicle in a Taylor-Green flow when $V_0=0.1$ and arrows indicate rotating directions. Figure~\ref{fig:BTG_Scale}e presents that the area $A/A_0$ decreases during the simulations and the inset shows constant total lengths $L/L_0$ for all cases where $A_0$ and $L_0$ are initial enclose area and midplane length.
%A/A0 plot with no monotonic trend.
One interesting finding is that the number of cells affects the directions of structure rotation.
...
\includegraphics[width=1.0\textwidth]{Figures/Figure8.pdf}
  \end{center}
  \vspace{-20pt}  
  \caption{\label{fig:BC2_shear}   Multi-lamellar stracture in shear flows. The insets of Fig~\ref{fig:relax}c is used for panels (a). Panels (b)-(d) are snapshots when shear rates are $\dot\gamma=\{0.05, 0.1, 0.15\}$ at $t=600$, respecitively. The streamlines are plotted in the background.  A multi-lamellar structure in shear flows with shear rates $\dot\gamma=\{0.05, 0.1, 0.15\}$. For this set of simulations, the local order parameter is a good indicator for tracking the rupture or structure separations. Moreover, all three measure show that the energy, orderparameter, and MSD increase as shear rate increases.
       }
\end{figure}

We place a 60-particle multilamellar configuration (inset of Figure~\ref{fig:BC2_shear}a) in shear flows where $\dot\gamma=0.05, 0.1, 0.15$. Quantities such as energies, orientational parameter, and positional parameter (Figure~\ref{fig:BC2_shear}d--f) follow the same trend where they increases as shear rate increases. 
...
\includegraphics[width=1.0\textwidth]{Figures/Figure10.pdf}        
  \end{center}
      \caption{\label{fig:BC3_shear}  A striated configuration in shear flows where the streamlines are plotted in the background. Panel (a) uses the insets of Fig~\ref{fig:relax}d and panels (b)-(d) are snapshots for the shear rates $\dot\gamma=\{0.1, 0.125, 0.15\}$ at $t=100$, respectively. 
       A striated configuration in shear flows with shear rates $\dot\gamma=\{0.1, 0.125, 0.15\}$. The energy profile, order parameter, and MSD for the case $\dot\gamma=0.15$ show that a rupture occurs during the simulation. For other two cases, all measures remain nearly constant.
    }
\end{figure}
...
\includegraphics[width=1.0\textwidth]{Figures/Figure11.pdf}            
  \end{center}
  \vspace{-20pt}  
  \caption{\label{fig:BC3_TG} A striated configuration in Taylor-Green flows where the streamlines are plotted in the background. Panel (a) uses the insets of Fig~\ref{fig:relax}d and panels (b)-(d) are snapshots for the flow strengths $V_0=\{0.2, 0.25, 0.3\}$ at $t=400$, respectively.}
\end{figure}
