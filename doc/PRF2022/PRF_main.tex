...
%particle to be 
may be partly hydrophilic and hydrophobic (polar and non-polar).
More generally the surface of a Janus particle may have a spatial distribution of hydrophobicity/hydrophilicity.
In a viscous solvent, in a quiescent flow, Janus particles often self-assemble into
...
Dynamic flows offer yet another dimension of control that emerge naturally
...
vesicles suspended in a viscous fluid (at zero Reynolds number) has
...
In this work, we employ the newly developed JP model using BIEs ~\cite{Fu2022_JFM} 
and tune the boundary conditions with energy normalization to study the dynamics of 
amphiphilic (BC(i) and (ii)) and dipolar (BC(iii)) JPs. To overcome the well-known 
...
Simulations include the self-assembly behaviors when the flow is absent and particle 
dynamics when JP are suspended in shear and TG background flows with various 
flow rates. The free energy profiles demonstrate that the relaxation process for 
...
appear in different starting setup.

We track the total free energy, the orientational parameter, and the strain parameter to comprehensively summarize and classify the effective properties of all three materials. 
...
shear rate cases provide the range when the critical shear rate for structure to break 
apart and major topological changes. As expected in all case studies, the free energy 
