...
\includegraphics[width=1.0\textwidth]{Figures/Figure2.pdf}
  ...
  Panel (a) shows that the free energy \eqref{eq:free_energy} decreases
    ...
    The near-equilibrium configurations depend on the initial configurations and the results show that for both $N_b=60$ and $N_b=198$ cases, bilayer and multi-lamellar configurations have similar trends in orientational parameters. The results of the striated configuration depend on the initial geometrical setup.
  }
\end{figure}
...
We vary the cell size by tuning the parameter $l$ in \eqref{eq:tg_flow}.  Figure~\ref{fig:BTG_Scale}(a)--(d) show the deformation of a vesicle in a Taylor-Green flow when $V_0=0.1$ and $t = 1\ \mu$s where arrows indicate rotating directions. Figure~\ref{fig:BTG_Scale}(e) presents that the area $A/A_0$ decreases during the simulations and the inset shows constant total lengths 
$L/L_0$ for all cases where $A_0$ and $L_0$ are initial enclose area and midplane length.
