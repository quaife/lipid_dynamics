...
configurations (inset of Figures~\ref{fig:relax}(b), (d), and (e))~\cite{supp}.
...
In TG flow, $\lambda$ controls the size of the $\pi \lambda \times \pi
...
\begin{equation}
\label{eq:capillary}
\mathrm{Ca} = \frac{\mu \dot \gamma R^3}{\kappa}
\end{equation}
...
stress overcomes elastic stress when $\mathrm{Ca} > 1$ and this yields $\dot
\gamma > 2 \cdot 10^{-2}$ ns$^{-1} = 2 \cdot 10^6$~s$^{-1}$. Thus our values
...
From the particle diffusion time $D/L^2$  
and advection time $L/u = (\dot\gamma)^{-1}$,
$D = 0.17\cdot 10^{-9}$~m$^2$~s$^{-1}$ and $L \sim 10$~nm, we 
obtain a P\'eclet number $\mathrm{Pe} = (D/L^2)/(L/u) \sim 60$,
suggesting that advection dominates the particle transport.
% L u = L^2 gammadot = 10 nm^2/ns 
% D = kBT/(6 pi mu c) = kBT/(6 pi mPa s c) 
%   = kBT/(6 pi 1e-3 N/m^2 1.25 nm) 
%   = 4 1e-12 N nm/(6 pi 1.25e-3 m^{-2} s N nm) 
%   = 0.17e-9 m^2/s
%  
% Pe = L u/D = 10 nm^2/ns/(0.17e-9 m^2/s)
%    = 59e9 1e-18 1e9 = 59
}
...
suspensions in Figure~\ref{fig:s2esummary}~\cite{supp}.
...
$F$, alignment order $S_2$, and strain $E$~\cite{supp}. The eight setups correspond
...
formulation and tumbling inherent to the background flows~\cite{supp}. To extract
...
Figures~\ref{fig:ulshraw}--\ref{fig:sttgraw}~\cite{supp}). The integral
...
In panel (a), the relative change in $\dot \gamma_*$ is linear in the
  ...
  of a vesicle is in a TG flow at $t=1$ $\upmu$s with $\lambda= 1,2,3,4$,
...
during rupture (see Supplementary movie~\cite{supp}). Our previous
...
values of $\lambda$ in Figure~\ref{fig:PramChange}(b)--(e)~\cite{supp}.

{\color{red} For $\lambda=1$~nm and $\lambda=2$~nm, multiple TG cells are inside the
...
{\color{red}Such dynamic permeability due to fluid-structure interactions is also found in the clogging of
...
\langle |h(\kk)|^2 \rangle = \frac{\kbt}{L_xL_y}\frac{1}{\kappa |\kk|^4},
\end{equation}
...
fluctuate \cite{D0SM00943A}.
Rather, because the wave vector $\mathbf{k}$ is inversely proportional
to the characteristic
size of $L_x$ and $L_y$, larger energies are required to fluctuate membranes
...
\begin{acknowledgments}
  We thank the anonymous referees for pointing out
  the P\'eclet number, capillary number, and fluctuation analysis.
  E.~Corona, M.~Rachh, S.~Jiang, and T.~Beck
  provided useful insights.
  B.Q.~acknowledges support from the Simons Foundation,
