...
We discretize each of the $\Gamma_i$, $i=1,\ldots,N_b$, at $N$
discretization points. The trapezoid rule is used to approximate each of
the integrals in equations~\eqref{eqn:mobility1}
and~\eqref{eqn:mobility2}, and the double-layer potential
equation~\eqref{eqn:SKIE}. When two bodies are close, the double-layer
potential becomes nearly-singular, and we replace the double-layer
potential with an interpolation-based nearly-singular quadrature
rule~\cite{qua-bir2014}. After discretizing and applying appropriate
quadrature rules, the result is a $(2NN_b + 3N_b) \times (2NN_b + 3N_b)$
linear system whose condition number is independent of $N$, and this
linear system is solved using matrix-free GMRES. To reduce the number of
GMRES iterations, a block-diagonal preconditioner is used.
