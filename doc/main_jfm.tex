...
apposing monolayers of about $8$ nm in radius. The norms of the translational and
...
Previous studies \cite{Zgorski2019, denOtter2007}
have considered inter-monolayer slip in lipid bilayers simulated by MD. 
...
To calculate $U$, we let $v_+(\mathbf{x})$ and $v_-(\mathbf{x})$ 
be the tangential velocity of the outer, respectively inner, leaflet.  Then 
\begin{equation}
U = \frac{1}{L}\int_{C_M}v_+(\mathbf{x})(1 - \delta \kappa) - v_-(\mathbf{x})(1 +  \delta \kappa) \, ds,
\end{equation}
where $ \delta $ is the distance from the leaflet centers to the midplane and $\kappa$ is the curvature. 
The factors $\pm  \delta \kappa$ are needed to project the leaflet velocities, defined on the particle centers,
...
several shear rates. 
The coefficients are 0.79 $\pm$ 0.3 pN ns nm$^{-3}$ which is in good quantitative agreement with values
previously reported in the literature (\cite{sch-vla-mik2010}
and~\cite{denOtter2007}). \cite{denOtter2007} give $b = 2.4 \times 10^6$
Pa m$^{-1}$ s = 2.4 pN ns nm$^{-3}$  from their MD study, for example. 
...
continuum  mechanics, changes in surface area give rise to a membrane tension
