...
This vesicle consists of $58$ disks with radius $r=0.5$ and the tank-treading behavior appears after obtaining an equilibrium state. 
...
\includegraphics[width=0.22\textwidth]{N116_ext_7500.eps}
  \caption{Two vesicles under an extensional flow where the dimensioneless shear rate $\dot\gamma=0.005$; The initial configuration is shown in panel (a). The background color indicates the magnitude of fluid pressure. From dark red to dark blue, it represents the the pressure varies from high to low values.
  }
    \label{figure5}
\end{figure}
...
\includegraphics[width=0.22\textwidth]{N116_shear_9000.eps}
  \caption{Two vesicles under a shear flow where the dimensioneless shear rate $\dot\gamma=0.005$; The initial configuration is shown in panel (a). The background color indicates the magnitude of fluid pressure. From dark red to dark blue, it represents the the pressure varies from high to low values.
  }
    \label{figure6}
\end{figure}

Figures~\ref{figure5} and \ref{figure6} show two sets of simulations. Starting from the initial configuration (shown in both panel (a)), panels (b)-(d) show the transitions of two vesicles moving under an extensional flow. The extensional flow is stretching along the $y$ direction and squeezing in $x$ direction. Hence two vesicles are first pushed toward each other  then separate along the streamline. With the same starting state, panels (b)-(d) in Figure~\ref{figure6} are the observed phenomena when two vesicles are in a shear flow $\dot x = \dot\gamma y$. An interesting event occurs in the simulation that a rotational behavior with an adhesive effect can be observed and tuned. One can notice that the starting 
state of two vesicles are both centered on the x-axis. If two center of mass positions are not at the same horizontal level, then the adhesion observed in the shear flow case will be absent when they are well separated in $y$-direction.
