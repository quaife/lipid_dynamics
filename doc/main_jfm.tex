...
\begin{figure}
\begin{center}
\includegraphics[width=11.32cm]{Figure0_Wrapper.pdf}
\end{center}
\caption{The diagram illustrates the domain and boundary conditions for the mobility problem formulation.}
\label{fig:figure0}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=11.32cm]{Figure1_Wrapper.pdf}
...
\end{center} 
  ...
  denoted as $C_m$. We see that the intermonolayer slip is largest
...
membrane. This work is highly applicable with fields in material science
...
short-range repulsion to avoid unphysical contact. Our method requires
calculations of the hydrodynamic stress tensor, and these are used to
estimate the membrane permeability constant $P$ and the membrane
stretching modulus $K_A$. With the use of suitable parameters, the
numerical results yield qualitative phenomena such as a tank-treading JP
vesicle. The reduced area $A^*$ decreases as the shear rate increases
...
friction. The inter-monolayer friction coefficient is determined by
calculating the tangential friction force and slip velocities with
respect to the bilayer midplane. The resulting friction coefficients are
...
parabolic flow. The chosen parameter values result in an asymmetric
slipper shape. While the initial reduced area of the JP vesicle is $A^*
\approx 1$, the equilibrium reduced area is $A^*=0.9$. A qualitative
agreement is made by comparing the JP vesicle shape with the
two-dimensional inextensible membrane in continuum modeling
simulations~(\cite{Kaoui09, dan-vla-mis2009, cou-kao-pod-mis2008}). An
...
We compare setups of two JP vesicles and two vesicles described with a
...
fast multipole method. Another research direction is to introduce
thermal fluctuations into the model. This will result in Janus particles
that include Brownian motion, and this is critical to understand
membrane diffusion. Finally, a more physical boundary condition for the
