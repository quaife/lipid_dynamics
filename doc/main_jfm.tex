...
\begin{abstract}
  ...
  The Janus particles interact with each other directly through a hydrophobic attraction potential (HAP)
  that has been shown to model lipid bilayer membranes. In addition they also interaction with each other indirectly through the hydrodynamic interactions.
  We adopt an efficient integral equation method for solving the screened
  Laplace equation (for the HAP) and the mobility of Janus particles in a viscous fluid. 
  Both the hydrophobic and hydrodynamic interactions between near
  touched boundaries are accurately captured within the integral formulation. Under a linear shear flow,  we observe the tank-treading deformation
  in a two-dimensional vesicle made of Janus particles for various values of the shear rate.
  ...
  interactions in shear and extensional flows. We find that the hydrodynamics of a  vesicle assembled from Janus particles is similar to that of
  a lipid bilayer vesicle often described by the Helfrich free energy of an inextensible elastic membrane.
...
simulations replicate  well-know vesicle hydrodynamics such as
tank-treading and inter-leaflet slippage in a shear flow and migration
in Poiseuille flow from continuum models. Furthermore we use the hybrid
continuum model to investigate permeability and rupture of a bilayer
...
\cite{Fu20} studied physical quantities for static JP vesicles for
various particle shapes. In the present study, we fix the particle shape
and vary the background flows. Specifically, the particles are circular
disks with diameter $l_0 = 2.5$~nm. The boundary
...
and director respectively. The particle diameter is the thickness of
monolayers and the director points in the direction of the hydrophobic
side of the JP. 

We use $\rho = 2 l_0$ for decay length, $\rho_0 = 0.2l_0$ for repulsion
length, $M=4.0$~$k_BT$ for repulsion strength, $\gamma = \text{4.1 pN
nm}^{-1}$ for interfacial tension, and $\mu = \text{1 cP} = \text{1 pN
ns nm}^{-2}$ for viscosity of room-temperature water. We
nondimensionalize the problem through the change of variables 
%\begin{equation}
...
JP vesicles that is suitably close to equilibrium. We start with an
assumed configuration of $N=58$ JP in the form of two, circular,
apposing monolayers of about $8$ nm in radius. The translational and
rotational velocities vanish exponentially with an approximate decay
rate $\sim 4.6$ $\upmu$s$^{-1}$. An equilibrium configuration is
therefore rapidly attained. This equilibrium configuration serves as the
initial data in the subsequent background flow simulations.
...
\end{center} 
\caption{\label{figure5} There is inter-monolayer slip present in the JP
  ...
  gives the slip velocity. With the exception of $\chi = 0.0035$, the
...
the vicinity of the vesicle. While large, these values are identical to
...
are able to slide past one another. This sliding is limited by viscous
friction of the aqueous gaps between particles and by the constant
unbinding and binding of particles pairs in apposing leaflets. The
phenomenon is consistent with prior MD results (\cite{Zgorski2019,
denOtter2007}).

Figure~\ref{figure5}a illustrates the process by tracking the distances
traveled by a pair of particles. In the left image, the blue and yellow
...
the tangential velocity of the outer leaflet is larger than that of the
inner leaflet. The difference in their velocities is the slip velocity
$U(t)$ and $s(t) = \int_0^t dU $ records the distance one leaflet has
slid past the other. 

Figure~\ref{figure5}b plots the sliding distance for various shear
rates. From the data, we define an inter-monolayer friction coefficient 
\begin{align}
  b = \frac{\langle F\rangle}{\langle L \rangle \langle U\rangle},
\end{align}
where $F$ is the tangential force jump, $L$ is the length of the
midplane $C_M$ (Figure~\ref{figure5}a), $U$ is the slip velocity, and
$\langle \cdot \rangle$ denotes the time average. We define the
tangential force jump $F$ to be the tangential shear force on the outer
leaflet minus the tangential shear force on the inner leaflet. However,
it does not make sense to integrate along a curve passing through the
particles.  To calculate the tangential force jump $F$, we instead let 
\begin{align}
F_h = \int_{\mathcal{C}_M^+} 
  \ttau \cdot \sigma \cdot \nnu \,\dif s - \int_{\mathcal{C}_M^-} 
 \ttau \cdot \sigma \cdot \nnu \,\dif s 
\end{align}
where $C_M^+$ and $C_M^-$ are target curves obtained by moving the
midplane curve $C_M$ (Figure~\ref{figure5}a) a distance $h$ outward,
respectively inward, along the unit normal. We sample $F_h$ for $h = \pm
1.5, \pm 2.5, \pm 3.5$ times the particle radius and define $F$ by
extrapolating to zero distance.
...
several shear rates. The coefficients generally lie within the range 1
to 6 pN ns nm$^{-3}$ which is in good quantitative agreement with values
previously reported in the literature (\cite{sch-vla-mik2010}
and~\cite{denOtter2007}). \cite{denOtter2007} give $b = 2.4 \times 10^6$
Pa m$^{-1}$ s = 2.4 pN nm$^{-3}$ ns from their MD study, for example. 
...
dan-vla-mis2009} have shown that the behavior of a vesicle in this
...
\caption{\label{figure2} In JP vesicles, tension and pressure relax as
  ...
  a spatially constant internal pressure 0.05~pN nm$^{-1}$ (red color,
...
\end{figure}
Finally, we show that the JP vesicle behaves as an extensible, permeable
membrane. Figures~\ref{figure2}b shows an initial, non-equilibrium JP
...
constant, positive internal pressure (0.05~pN nm$^{-1}$) and the white
shows a constant zero, external pressure. There is some fluid flow and
the pressure vanishes as the configuration tends toward the equilibrium
state (Figures~\ref{figure2}c).

What could be the source of this drop in pressure? In continuum membrane
mechanics, changes in surface area give rise to a membrane tension
...
between the particle center and the far-field (see
...
size. The data give $K_A = 170 \pm 9$ pN nm$^{-1}$ which is good
agreement with the area moduli of lipid bilayer as determined by
experiment. To evaluate how changes to particle size lead to different
...
usual 2.5 nm. These data give smaller stretching modulus of $K_A =
102$~pN~nm$^{-1}$. 
...
respectively. Like the area modulus, permeability is independent of
vesicle size.
...
vesicle moves toward the center of the shear flow. While the initial
...
above the center of flow and exhibits tank-treading motion.
%A qualitative
...
vesicle oscillates at a height slightly above the center of the
Poiseuille flow. 
...
level. We also observe a rotating behavior that is observed for models
