...
between proximal particles. Given a pair particles indexed as $i$ and
...
%
  ...
  T_i^{\text{repul}} &= \sum_{j \neq i} 
    ...
    P'(|\tilde{\xx}_i - \tilde{\xx}_j|) 
\end{align}
where $\aa_i$ is the center of particle $i$. The repulsion profile
$P(r)$ is zero for distances $r$ larger than a repulsion length scale
$\rho_0$. As such,~\eqref{eqn:REPULforce} and~\eqref{eqn:REPULtorque}
ignore particles $\Gamma_j$ outside a tubular neighborhood of
$\Gamma_i$. For $0 \leq r < \rho_0,$ we use $P(r) = M(1 -
\sin(r/\rho_0))$ where $M$ is sufficiently large to prevent particle
collisions. The total imposed force and torque are
\begin{align}
