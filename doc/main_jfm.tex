...
\end{center} 
 ...
 JP vesicles loose enclosed area and conserve length in shear flow. 
  Panel (a) gives the reduced area over time
  for four different shear rates. The smooth curves are a best-fit
  exponential curve. There is commensurate increase in excess length (panel (b)), 
  but the arc length of the mid plane curve is more or less constant.   
   The legend in panel (b) applies to panels (a)--(c).
  Panel (d) plots the steady-state reduced area and the decay rate $k$ (inset) using the exponential fit
  to the data in panel (a). }
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center} 
  ...
  Panel (a) tracks the angle  $\theta(t)$ between a pair of particles (blue and yellow) with respect to
  the JP vesicle centroid. The shear rate is $\chi=0.005$ and in the right figure, the yellow particle
  will complete two and a quarter revolutions in the same time the blue particle completes two revolutions.
  The curves in panel (b) are the mean change in angle for various shear rates. 
  Slip velocity generally increases with shear rate but 
  we have marked two outliers (times symbols). }
\end{figure}
...
particle suspension to stretch apart. This perturbation  
...
Due to granularity, the tank-treading motion has some wobble and this is the source
of the oscillations in the panels of Figure~\ref{figure4}.
...
The curves in panel (a) are fit of the exponential model $A^* = A^*_{\infty} + (1-A^*_{\infty}) \exp(-kt)$.
Panel (d) shows that $A^*_{\infty}$ decreases with shear rate, 
and that reduced area converges to a steady state earlier when the shear rate is lower (inset).
...
which gives fluid velocities like m s$^{-1}$ in the vicinity of the  vesicle.  
While seemingly large, these values are consistent with prior MD studies (\cite{Brandner2019})
and are a consequence of the fact that the magnitude of elastic membrane stresses grows inversely 
with the vesicle radius; larger shear rates are required to produce the viscous stresses needed to 
...
The two leaflets of the vesicle are not bound together, but rather are able to slide 
...
Figure~\ref{figure5}a illustrates the process by tracking the angle $\theta(t)$
between two particles, one blue and the other yellow, relative to the vesicle centroid. 
It is clear from the growth in angle that the outer leaflet has a tangential velocity that is larger  
then that of the inner leaflet.  
Figure~\ref{figure5}b plots the mean, particle-pair angle for various shear rates. 
It suggests that the inter-monolayer slip velocity generally increases with shear rate.  

From the data, we are able to define an inter-monolayer friction coefficient  
\begin{align}
...
To measure inter-monolayer slip $U$, we compute the mean tangential velocity of the particles in the outer monolayer
and subtract from it the mean tangential velocity of the particles in the inner monolayer. 
To calculate the tangential force jump $F$, we let 
\begin{equation}
...
a distance $h$ along the outward, respectively inward, unit normal.  Actually, we calculate $F_h$ for several values of $h$ 
and then define $F$ to by extrapolating to zero distance giving a jump in tangential shear force across the midplane.  
...
$b$ (pN$\cdot$ns nm$^{-3}$)    & $4.31\times10^1$ &  $1.45\times10^2$  & $1.57\times10^2$ &  $2.14\times10^2$ &  $2.3\times10^2$ &  $2.46\times10^2$ &  $2.67\times10^2$\\ 
\hline    
\end{tabular} 
\label{table1}
\end{table}
...
range 0.2 to 0.3 pN ns nm$^{-3}$ which is in good quantitative agreement with
values previously reported in the literature (\cite{sch-vla-mik2010}
and~\cite{denOtter2007}).  
The data contain two outliers (Table~\ref{table1}, * symbol).
These correspond to the two outliers in the slip velocities (Figure~\ref{figure5}b, times symbol). 
We suspect that due to the finite particle size, the granular wobble of the tank-treading motion is more prominent for some
shear rates, causing some slip velocities to be lower than expected.  
...
\caption{\label{figure2} 
  ...
  At $t = 0$ $\upmu$s, the vesicle has a constant internal pressure 0.05~pN nm$^{-1}$ (red color, panel (b)).
