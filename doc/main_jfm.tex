...
The governing equations of $N_b$-body particles suspended in the
two-dimensional, unbounded
domain $\Omega$ are
\begin{alignat}{3}
...
\end{alignat}
%
where $\uu$ is the velocity, $\mu$ is viscosity, $p$ is the pressure, and $\uu_\infty$ is
the background flow. Each particle $\Gamma_i$ with center $\aa_i$ is
a rigid body, and its velocity satisfies 
\begin{align}
...
angular velocity.
Here, $\langle x, y \rangle^{\perp} = \langle -y, x \rangle$.
Therefore, the no-slip boundary condition of
the velocity field is 
\begin{align}
  \uu(\xx) = \vv_i + \omega_i (\xx - \aa_i)^\perp, \quad
    \xx \in \bd\Gamma_i.
\end{align}
To determine the translational and angular velocities of each rigid body,
we assume some imposed forces $\FF_i$ and torques $T_i$ acting on each particle.
Since the small particles are intertialess, force balance gives 
\begin{alignat}{2}
  \label{eqn:force}
...
The imposed forces and torques contain two parts. 
The first part comes from the hydrophobic attraction potential (HAP).
This potential was introduced in \cite{Fu20} and
models hydrophobic attraction by solving the screened Laplace equation boundary value problem
\begin{alignat}{2}
  \label{SL}
-\rho^2 \Delta u + u &=0,            && \mathbf{x} \in \Omega,\\
\label{SLbc}
u(\mathbf{x}) &= f_i(\mathbf{x}),\qquad  && \mathbf{x} \in \Gamma_i, \\
\label{SLff}
u &\to 0,                          &&|\mathbf{x}| \to \infty,
\end{alignat}
where $f_i$ is material label with $f_i = 0$, resp. $1$ representing hydrophilic, resp. hydrophobic, 
portions of the surface, and $\rho$ is the decay length of attraction.
The forces and torques for this attraction are 
\begin{align}
  \label{eqn:hydrophobicAttraction}
  \FF_i^{\text{hydro}} = \int_{\bd \Gamma_i} {\bf T}\cdot \nnu_i \, dS, 
    \quad 
...
= \gamma\rho^{-1}u^2 \mathbf{I} + 2\rho\gamma \left(\tfrac{1}{2}|\nabla
  u|^2 \mathbf{I} - \nabla u\otimes \nabla u\right)
\end{equation}
is the hydrophobic stress tensor.  The parameter $\gamma$ is interfacial tension. 
The forces $\FF_i^{\text{hydro}}$ and torques $T_i^{\text{hydro}}$ are responsible for
forming particle aggregates that partially sequester 
the hydrophobic portions of the particle surface.

The second part of the imposed forces and torques comes from repulsion between proximal particles.
We define 
\begin{equation}
  \label{REPUL}
  \FF_i^{\text{repul}} = \sum_{i=1}^{N_b} \frac{\mathbf{x}_{ij} - \mathbf{y}_{ij}}{\rho_{ij}} P(\rho_{ij}),\quad 
  T_i^{\text{repul}} = \sum_{i=1}^{N_b} (\mathbf{x}_{ij} - \mathbf{a}_i)^{\perp}\cdot \frac{\mathbf{x}_{ij} - \mathbf{y}_{ij}}{\rho_{ij}} P(\rho_{ij})
\end{equation}
where $\mathbf{x}_{ij} \in \partial \Gamma_i$, 
$\mathbf{y}_{ij} \in \partial \Gamma_j$, 
and $\rho_{ij} = |\mathbf{x}_{ij} - \mathbf{y}_{ij}|$
is the distance between the pair of particles.
The repulsion profile $P(r)$ is zero for distances $r$ larger than a repulsion length scale
$\rho_0$.  As such, \eqref{REPUL} ignores particles $\Gamma_j$ outside a tubular neighborhood
of $\Gamma_i$.  For $0 \leq r < \rho_0,$ we use $P(r) = M(1 - \sin(r/\rho_0))$ where $M$ is sufficiently
large to prevent particle collisions.  The total imposed force and torque are
\begin{equation}
  \FF_i = \FF_i^{\text{hydro}} + \FF_i^{\text{repul}},\quad
    T_i =   T_i^{\text{hydro}} +   T_i^{\text{repul}}.
\end{equation}


\subsection{Time marchning and Nondimensionalizations}
From the mobility problem, we obtain translational and angular velocities of the $N_b$-body system.
A second order Adam-Bashforth scheme updates the particle positions and orientations.
dynamics but body collisions may occur during the simulation while a relatively large time step is chosen.
We therefore impose a short range repulsion that it will be turned on when the distance between two boundary points on different bodies are within one characteristic length.
...
\todo[inline]{Talk about how to solve numerically. Quadrature, time
stepping, collocation, etc.}
...
\end{center} 
  \caption{For the shear rate $\dot\gamma=0.005$, the inter-monolayer slip is be observed. The curve in black represents the mean angle between each particle pair where the angle is calculated from the relative position of particles using the vesicle center of mass position. The configuration plots have two fixed particles marked in yellow and blue and labeled with the corresponding time $t=\{2000,5200,9000\}$.
  }
    \label{figure5}
\end{figure}
