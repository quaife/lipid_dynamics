...
The color map in panels (b) - (e) is the solution $u$ of \eqref{eq:SL}. Red is for $u = 1$
and blue is for $u = 0$.
Particles self-assemble into vesicle bilayers (panel (b)) and eventually  
arrange along inner and outer leaflets. In panel (c), $C_M$ is the midplane curve,
$A$ is the enclosed area, and $L$ is the arc length.  Initially stretched vesicles relax to equilibrium 
...
The objective is to study the hydrodynamics of JP vesicles in background flows. 
We consider an $N_b$-many body collection of JP suspended
in a two-dimensional unbounded domain $\Omega$. 
The boundary of each particle is denoted by $\Gamma_i$ so that $\bd \Omega = \Gamma_1 \cup
\Gamma_2 \cup \cdots \cup \Gamma_{N_b}$ (Figure \ref{fig:figure0}a). 
Assuming the inertial terms are negligible, the governing equations are
\begin{alignat}{3}
  -\mu \Delta \uu + \nabla p &= \mathbf{0}, 
    && \xx \in \Omega, \\
...
particle. Since the small particles are inertialess, force balance
gives 
\begin{alignat}{2}
  \label{eq:force}
...
The imposed forces and torques contain two parts: hydrophobic attraction and repulsion. 
The hydrophobic attraction potential was introduced by ~\cite{Fu20} and is responsible for forming particle aggregates
that sequester their hydrophobic surface regions (Figure \ref{fig:figure0}b).
We model hydrophobic attraction by solving
...
discretizing~\eqref{eq:screenedSKIE}, the result is an $NN_b \times NN_b$
...
with block-diagonal preconditioned GMRES. Other BIE
formulations of the mobility problem have used single-layer potentials
...
zero for a single particle (see \cite{Fu20} equation 2.13). The angle of the ellipse's major axis coming
from the integral equation method was in complete agreement with the
theoretical, Jeffery orbit time-course (\cite{jef1922}). 
...
\end{align}
\end{theorem}
The symbols $\ttau$ and $\frac{d}{ds}$ are the unit tangent and arc length
derivative for $\Gamma_i$, respectively (Figure \ref{fig:figure0}a.)  
The result 
is valid for any smooth particle shape or boundary condition. 
...
$8$ nm in radius. 
Figure~\ref{figure2}a shows that the 
...
in the background flow simulations.
...
outside of the vesicle (\cite{Finken08, Shaqfeh11}).
...
\end{center} 
  ...
  will complete two and quarter revolutions in the same time the blue particle completes two revolutions.
