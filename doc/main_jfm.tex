...
\ReviewerTwo{and empitically deduced realistic bending moduli, stretching moduli,
...
%
\ReviewerOne{The trends in Table~\ref{table1}
...
the dimensionless, critical shear rate is $\chi_*=0.0655$ so that the membrane ruptured whenever $\chi > \chi_*$
  ...
  A 5 \% decrease in the particle radius $c$, screening length $\rho$ gave a $-9.55$ \% decrease, $5.0 \%$ increase
  respectively in $\chi_*$.
  Finally, a 10 \% increase in resting length $L_0$ gave a 20.61 \% decrease in critical shear rate $\chi_*$.}  
  \ReviewerOne{These relative changes suggest that there is dimensionless number $K$ given by 
  \begin{equation}
    \label{eq:CSR}
  ...
  $\text{Ca} = K c^2/(\rho L)$. Based on the above data, $\text{Ca}$ would be approximately $1.7$
  so that surface forces are comparable to viscous forces.  
  There is a related \emph{elastic} capillary number
  $\tilde{\text{Ca}} = \mu \dot{\gamma} R_0^3/K_B$
  ...
  \]
  which yields $K_B \approx 60$ pN $\text{nm}^{-1}$, which is a reasonable
...
%\includegraphics[width=0.8\textwidth]{slipper.eps}
  ...
  \ReviewerThree{The horizontal dashed line represents time in units of
...
\subsection{\ReviewerOne{Stretching and Permeability}}
...
\begin{figure}
  ...
  The value $d_{1,2}$ is the distance between two centroids of JP vesicles. The thick curves plot the trajectories
...
fluids requires an introduce of a contact line between
the two phases.
...
fast multipole method. Oother research directions include
