...
The length unit is scaled by the length of phospholipids $l=2.5$ nm and we set the force is scaled from the 
interfacial tension $\gamma=4.1$ pN nm. We then be able to calculate the corresponding 
time scale $\tau = 0.38$ ns. With the same framework, \cite{Fu20} provides the bending rigidity 
$\kappa=8.51$ kT $\approx35$ pN nm.

For the $N$-body Janus-vesicle, we denote a constant initial radius $R_0$ and the dimensionless shear rate 
is referred from~\cite{Finken08}
\begin{align}
  \chi = \dot\gamma \cdot\frac{\mu R_0^2}{\kappa},
\end{align}
%
where $\dot\gamma$ the dimensional shear rate, and $\mu=1$ cP the fluid viscosity is used through this work.
