...
interfacial tension $\gamma=4.11$ pN nm$^{-1}$ from ~\cite{Ryham16}. We then be able to calculate the characteristic time scale $\tau = \mu l/\gamma = 0.61$ ns where the fluid viscosity $\mu=1$ cP. With the same framework, \cite{Fu20} provides the bending rigidity $\kappa=8.51$ $k_BT$.
%
The dimensionless shear rate is given by $\chi = \dot\gamma \tau$ where $\dot\gamma$ 
is the shear rate.
