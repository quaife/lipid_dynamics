...
necessary to have a suitable, initial, equilibrium vesicle.  This is
...
$8$ length units. The directors of outer monolayer point inward and the
directors of the inner monolayer point outward (Figure~\ref{figure2}a).
The JP of this chosen configuration are generally not in equilibrium.
Rather, the inter-particle distances and orientations must make slight
adjustments so that attraction and repulsion are balance.
Figure~\ref{figure2}(b) shows that the magnitudes of the mean velocity
components ${\bf v}=\{v_x,v_y\}$ and the variation $\Delta \theta$ of
the particle orientation decrease exponentially with an approximation
decaying rate $\sim0.005$.  A local energy minimum and the equilibrium
configuration is therefore rapidly attained.  The equilibrium
configuration in Figure~\ref{figure2}a is slightly elliptical.  The
equilibrium eccentricity vanishes for larger $N_b$
...
stretching modulus.  The permeability for the 117 particle number case
was larger, 0.0566 nm$^3$ ns$^{-1}$ pN$^{-1}$.  That is because
hydrophobic attraction vanishes with particle size, leading to larger
gaps between the particles for the same repulsion.  The hydraulic
permeability we calculate, however, is not in agreement with
...
Since the vesicles in Figure~\ref{figure2}a are nearly we circular, we
can combine~\eqref{eq:stretch},~\eqref{eq:LaplacePressure},
...
is applied. Snapshots of the JP vesicle in a shear flow with a shear
...
configurations of the JP vesicle at time $t=4000$ are in panels (c)
...
velocity around the JP vesicle. In panel (d), we can clearly observe
...
\includegraphics[width=0.3\textwidth]{N58_vel_20000.pdf}
  \caption{\label{figure3} A JP vesicle suspended in a shear flow
  with shear rate $\chi=0.0025$: (a) Initial configuration of 58-body
  JP vesicle where black arrows are the particle orientations. (b)
  ...
  represent the particle velocities. (c) Deformed JP vesicle at
  $t=4000$. At this configuration the JP vesicle is tank-treading.
...
where $A$ the area, and $L$ the total length of the JP vesicle, and
the excess length is $\Delta=L/\sqrt{A^*/\pi}-2\pi$.
Figure~\ref{figure4} shows these quantities for a JP vesicle
...
flow. $R_0$ is defined as the radius of the JP vesicle at $t=0$.
...
\includegraphics[height=2in]{parabolic.eps}
  \caption{\label{figure7} The schematic of a JP vesicle suspended in
