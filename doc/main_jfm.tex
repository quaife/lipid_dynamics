...
The preliminary test of the proposed model using integral equation method is the Jeffery orbits of single ellipsoidal particle under a shear flow $\dot x = \dot\gamma y$ where $\dot\gamma$ is the shear rate. A particle is initially placed with its center of mass fixed at the origin and the angle between the major axis and the $y$-axis is denoted as $\chi = 0^\circ$. Based on the theoretical Jeffery orbits with respect to the target angle $\chi(t)$, 
...
For the target bilayer, a circular initial shape is chosen to perform the relaxation run. This vesicle consists 
of $N=58$ disks with radius $r=0.5$ and the starting diameter for the vesicle is $\sim8.5$.
The starting center of mass position of the vesicle is pinned at the origin and we track particle velocities 
to determine the initial configuration for later simulations. This will prevent some numerical instability issues when the minimum of the total energy is not achieved. For instance, the dynamics of the bilayer under a shear flow may reach different outcomes.
Figure~\ref{figure2} shows that the magnitudes of velocity components ${\bf v}=\{v_x,v_y\}$ decrease exponentially with an approximation decaying rate $\sim1/3.74N$. The time step size 
$\Delta t=0.2$ is used for all simulations including the relaxation run.

\begin{figure}
\centering
\includegraphics[width=0.4\textwidth]{relax.eps}
  \caption{Vesicle relaxes under a quiescent flow. The blue dashed curve and the red solid curve are mean magnitudes of the particle velocities. The black dashed line shows the reference of how velocity magnitudes 
  decay.
  }
    \label{figure2}
\end{figure}
...
The snapshots of the simulation for single vesicle in a shear flow is shown in Figure~\ref{figure3}.
Panel (a) shows the initial circular shape obtained from the relaxation run and the deformed 
configuration of the vesicle after a period of time is in panel (c). The color presented in the figure 
...
To demonstrate the tank-treading behavior of the vesicle, panels (b) and (d) provide the fluid velocities 
over the whole computational domain when the shear flow passes through the vesicle.
...
\includegraphics[width=0.3\textwidth]{N58_vel_20000.pdf}
  ...
  (b) The fluid velocity plot with respect to the configuration in (a);
  ...
  The background colors in panels (a) and (c) represent the strength of the hydrophobic attraction potential. The blue region shows no action and the red region with the bilayer gives the strongest 
  action.
  }
    \label{figure3}
\end{figure}
...
The results of the reduced area are shown in Figure~\ref{figure4} where we compare the change of area over the time with a set of shear rates $\dot\gamma=\{0.002,0.0025,0.003,0.0035\}$. The reduced area is given by $A^* = 4\pi A/L^2$ where $A$ is the vesicle area and $L$ is the arc length of the vesicle with respect to the midplane. Since the starting configuration is close to a circular shape, the reduced are decreases from a value very close to $1$. It is clear that a slower shear rate generates a higher reduced area and has faster convergence. The small oscillations in all results are due to the granular setup of the bilayer which tends to having a slightly non-smooth curve in simulations.

\begin{figure}
\begin{center}
\includegraphics[width=0.5\textwidth]{ReducedArea.pdf}
\end{center} 
  \caption{Reduced area transitions over time. 
...
\includegraphics[height=1.2in]{N116_ext_7500.pdf}
  \caption{Two vesicles under an extensional flow where the dimensioneless shear rate $\dot\gamma=0.005$; The initial configuration is shown in panel (a). The background color indicates the magnitude of fluid pressure. From dark red to dark blue, it represents the the pressure varies from high to low values. The color bar is valid for all panels.
  }
    \label{figure5}
\end{figure}
...
Figures~\ref{figure5}-\ref{figure6} show the numerical results of simulations when two vesicles are in an extensional flow. Starting from the initial configuration (shown in panel (a)), panels (b)-(d) show the transitions of two vesicles moving under an extensional flow. The extensional flow is stretching along the $y$ direction and squeezing in $x$ direction with shear rate $\dot\gamma=0.005$. Since two bilayers are close enough
...
The pressure plots in Figure~\ref{figure6} provide a realistic result where the pressure is much higher in the gap between two nearby vesicles.
...
Two vesicles in a shear flow is resented in Figures~\ref{figure7}-\ref{figure8} where centers of mass are placed on the same horizontal level. With the same starting state as one in the extensional flow, panels (b)-(d) in Figure~\ref{figure8} are the observed phenomena when two vesicles are in a shear flow where the flow is given by $\dot x = \dot\gamma y$. 
...
\end{equation}
%
where $v_{max}$ is the flow strength and $w$ controls the shape of the flow. $R_0$ is defined as the radius of the initial vesicle midplane. Figure~\ref{figure9} gives a final configuration for one specific case where the circular vesicle is initially placed above the $x$-axis.
...
\begin{figure}
\centering
\includegraphics[width=0.3\textwidth]{N58_rupt_0.eps}
\includegraphics[width=0.3\textwidth]{N58_rupt_200.eps}\\
\includegraphics[width=0.3\textwidth]{N58_rupt_400.eps}
\includegraphics[width=0.3\textwidth]{N58_rupt_600.eps}
