...
fluid velocity over the whole computational domain when the shear flow passes through the vesicle. The tank-treading motion of the structure is observed.
...
In order to extract more physical properties of the proposed model, we track the total length of the bilayer structure and the reduced area over the time.
These results are shown in Figure~\ref{figure4} where we compare output the results from a set of shear rates $\dot\gamma=\{0.002,0.0025,0.003,0.0035\}$. 
...
\begin{figure}
\begin{center}
\includegraphics[width=0.45\textwidth]{ArcLength.pdf}
\includegraphics[width=0.45\textwidth]{ReducedArea.pdf}
\end{center} 
  \caption{(a) Total arc lengths of the outer leaflets over time in different shear rates; (b)Reduced area transitions over time. 
...
Figures~\ref{figure5} show the numerical results of simulations when two vesicles are in an extensional flow. Starting from the initial configuration (shown in panel (a)), panels (b)-(d) show the transitions of two vesicles moving under an extensional flow. The extensional flow is stretching along the $y$ direction and squeezing in $x$ direction with shear rate $\dot\gamma=0.005$. Since two bilayers are close enough
...
Two vesicles in a shear flow is resented in Figures~\ref{figure6} where centers of mass are placed on the same horizontal level. With the same starting state as one in the extensional flow, panels (b)-(d) in Figure~\ref{figure8} are the observed phenomena when two vesicles are in a shear flow where the flow is given by $\dot x = \dot\gamma y$. 
...
\end{equation}
%
where $v_{max}$ is the flow strength and $w$ controls the shape of the flow. $R_0$ is defined as the radius of the initial vesicle midplane. Figure~\ref{figure7} gives a final configuration for one specific case where the circular vesicle is initially placed above the $x$-axis.
