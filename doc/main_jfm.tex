...
two-dimensional unbounded domain $\Omega$. The boundary of each particle is
denoted by $\Gamma_i$ so that $\bd \Omega = \Gamma_1 \cup \Gamma_2 \cup
\cdots \cup \Gamma_{N_b}$ and $\nnu_i$ is the particle outward normal. 
Assuming the flow is Stokesian, the governing
equations are
\begin{alignat}{3}
  -\mu \Delta \uu + \nabla p &= \mathbf{0}, 
    && \xx \in \Omega, \\
...
Once \eqref{eq:SKIE} has been solved and $\sigma$ determined,
we can evaluate the integrals  \eqref{eqn:hydrophobicAttraction}. 
These integrals involve the stress \eqref{eq:stress} which contains
a singular integral for the gradient of the double-layer potential.   
To avoid singular integrals, we use the alternative forms
\begin{equation}
\label{eq:recipforcetorque}
\FF_i^{\text{hydro}} = \int_{\Gamma_i} \JJ_i \,\dif s,\quad
T_i^{\text{hydro}}    = \int_{\Gamma_i} (\xx - \aa_i)^{\perp} \cdot \JJ_i  \,\dif s
\end{equation}
where
 \begin{equation}
...
\end{equation}
Here, $\ttau$ and $\frac{d}{ds}$ are the unit tangent  and arc length  derivative for $\Gamma_i$, respectively, 
and
\begin{equation}
\label{eq:uidef}
    ...
    \quad \xx \in \Omega.
\end{equation}
The advantage in using \eqref{eq:jumpstress1} over \eqref{eq:stress} 
is that components $\sigma$, $v_i$, $\frac{d\sigma}{ds}$, and  $\frac{dv_i}{ds}$ are all smooth.  
...
\end{equation}
Using the fact that $u$,  $u_i$, and $v_i$ solve the equation $-\rho^2 \Delta u + u = 0$,
and that $\mathbf{T}$, $\mathbf{T}_j$, $j = 1, 2, 3$ are symmetric, it is straightforward to verify that 
  \begin{equation}
...
Let $U_i$ be the interior of the particle indexed by $i$. 
For $\xx_0 \in \Gamma_i$ and an arbitrary function $g(\xx)$, the notation
\begin{equation}
\lim_{\xx \to \xx_0^+}g(\xx),\quad 
\lim_{\xx \to \xx_0^-}g(\xx), \quad
[g](\xx_0) = \lim_{\xx \to \xx_0^+}g(\xx)  - \lim_{\xx \to \xx_0^-}g(\xx)
\end{equation}
denotes the limit taken from points in $U_i^c$,  the limit taken from points in $U_i$, and the jump, respectively.  
\begin{lemma}
\begin{equation}
  \label{eq:prejump}
\FF_i = \int_{\Gamma_i} [{\bf T}_2  \nu_i]  \, \dif s,\quad
T_i = \int_{\Gamma_i} (\xx - \aa_i)^{\perp} \cdot [{\bf T}_2 \nu_i] \, \dif s.
\end{equation}
\end{lemma}
\begin{proof}
...
\end{equation*}
By \eqref{eq:uidef} and \eqref{eq:decompdivfree}, we have $u_i$ smooth and  
$\nabla \cdot {\bf T}_1 = \mathbf{0}$ in $\mathbb{R}^n \setminus U_i$.
Similarly, $v_i = u - u_i$ is smooth and
$\nabla \cdot {\bf T}_3 = \mathbf{0}$ in $U_i$. 
By the divergence theorem,  
\begin{equation*}
  \int_{\Gamma_i}  {\bf T}_1\nnu \,\dif s
  = -\int_{\mathbb{R}^n \setminus U_i} \nabla \cdot {\bf T}_1 \,dx = \mathbf{0},\quad
    \int_{\Gamma_p}  {\bf T}_3 \nnu\,\dif s
  = \int_{U_p} \nabla \cdot {\bf T}_3 \,dx = \mathbf{0}.
\end{equation*}
Finally, $\nabla \cdot {\bf T}_2= 0$ in $U_i$. This gives
\[
\mathbf{0} = \int_{U_i} \nabla \cdot {\bf T}_2 \,dx = -\int_{\Gamma_i}  ({\bf T}_2 \nnu)^-\,\dif s,
\]
...
\end{align}
Using \eqref{eq:DLjump}, \eqref{eq:DLdevjump}, \eqref{eq:Tsplit}, and that $v_i$ is smooth in a neighborhood of $\Gamma_i$ gives
    \begin{align*}
    [{\bf T}_2\nnu]   &= [(\mathbf{S}(u_i,v_i)  +\mathbf{S}(v_i,u_i))\nnu] \\
    &= [ ( 2\gamma\rho^{-1} u_i v_i \mathbf{I} + 2\gamma\rho \nabla u_i \cdot \nabla v_i \mathbf{I} 
...
%Two vesicles in a shear flow is presented in Figures~\ref{figure8} where centers of mass are placed on the same horizontal level, at $(-10,0)$ and $(10,0)$. With the same starting state as one in the extensional flow, panels (b)-(d) in Figure~\ref{figure8} are the observed phenomena when two vesicles are in a shear flow where the flow is given by $\dot x = \chi y$. 
