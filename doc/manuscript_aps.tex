...
\subsection{Hydrophobic Attraction Potential}
...
\begin{equation}
\label{eq:main}
\Phi(\Omega,f) = \gamma  \min_{u \in \mathcal{A}} I[u],
 \end{equation}
...
{\bf v}_i + &\omega_i\times({\bf x}-{\bf a}_i)\\
 =& -\frac12\sigma_{hyd}({\bf x}) + D[\sigma_{hyd}]({\bf x}) \\
&- \sum_{i=1}^N\big({\bf S}({\bf x}, {\bf a}_i){\bf F}_i+ {\bf R}({\bf x}, {\bf a}_i)\tau_i\big)+{\bf u}_\infty, \quad {\bf x}\in\Gamma \\
\end{aligned}
\end{equation}
%
\begin{gather}
\int_{\partial M_i}\sigma_{hyd}\cdot {\bf n} ds = {\bf 0}\\
	\int_{\partial M_i}\sigma_{hyd}\times({\bf x}-{\bf a}_i)\cdot {\bf n} ds ={\bf 0}
\end{gather}
where 
\begin{equation}
\begin{aligned}
{\bf S}({\bf x}, {\bf a}) &= \frac{1}{4\pi}\left(-\log \rho{\bf I}+\frac{{\bf r}\otimes{\bf r}}{\rho^2}\right), \quad \text{and} \\ {\bf R}({\bf x},{\bf a})&=\frac{{\bf r}^\perp}{4\pi\rho^2}, \\
&{\bf r}={\bf x}-{\bf a}\quad  \text{and} \quad \rho=|{\bf r}|
\end{aligned}
\end{equation}
%
