classdef poten_yukawa 
% this class defines single and double layers for various kernels
% (stokes, laplace, yukawa) on 2D periodic curves.
...
% D = yukawaDLmatrix(geom), generate double-layer potential for 
% yukawa. geom is a data structure defined as in the capsules class
% D is (N,N,n) array where Np is the number of points per curve and 
...
%   % tangent at sources
  ...
  const = -(diffx.*normal(:,1) + diffy.*normal(:,2));
  ...
  omega = 1i*geom.rho;
  tmp=omega*r12;
  kernel = -1i/4*tmp.*besselh(1,tmp);  


  D11 = kernel./r2.*const.*sa;
  D11(1:Ng+1:Ng^2) = cur/2/pi.*sa(1,:);
  D(:,:,k) = D11;
...
%   % tangent at sources
  ...
  const = (diffx.*normal(:,1) + diffy.*normal(:,2))./r2;

  omega = 1i*geom.rho;
  tmp=omega*r12;
  kernel1 = 1i/4*omega^2*besselh(0,tmp);  
  kernel2 = 1i/4*omega*besselh(1,tmp)./r12;
  kernel3 = -1i/2*omega.*besselh(1,tmp)./r12;

  GradD11 = ( (kernel1+kernel3) .*(diffx).*const + ...
             kernel2.*normal(:,1)).*sa;
  % (1,1) component
...
%   pause  
  ...
  GradD22 = ( (kernel1+kernel3) .*(diffy).*const + ...
             kernel2.*normal(:,2)).*sa;
%   (2,2) component
...
% DLP = exactYukawaDLdiag(geom,f,K) computes the diagonal term of
...
% VESICLE DUE TO ALL OTHER VESICLES (ex. yukawaSLP) AND CAN
...
if nargin == 6
  Ntar = size(Xtar,1)/2;
  ncol = size(Xtar,2);
  yukawaDLPtar = zeros(2*Ntar,ncol);
else
  K1 = [];
  yukawaDLPtar = [];
  ncol = 0;
