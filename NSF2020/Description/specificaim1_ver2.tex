...
\end{wrapfigure}
We propose to {\bf (i)} determine the value of effective elastic moduli $\KB$,
...
shows an example where the initial configuration of a bilayer
patch containing the splay deformation without any other components
(saddle splay, twist and tilt). As the bent membrane flattens, 
both the self-interaction energy $\Phi$ and the elastic
...
for the HK functional \eqref{ansatz3} is that surface-director coupling makes it
...
and with it we can help explain unexcepted phenomena like hierarchical self-assembly in
colloidal systems \cite{Luo2019}.
