...
/*
	...
	{
                cout<<"intersection: "<<idx<<", vertex: "<<iItr->getVertex()<<", edge v1 "<<iItr->getEdgeVertex1()<<", edge v2 "<<iItr->getEdgeVertex2()<<", alpha "<<iItr->getAlpha()<<", time "<<iItr->getTime()<<endl;
...
*/
	...
	//sort intersections into disjoint regions	
	vector<Intersections> intersections;
	groupIntersections(is,intersections);
	int mycount = 0;
	for (vector<Intersections>::iterator isItr=intersections.begin(); isItr!=intersections.end(); ++isItr)
	{
		size_t idx = 0;
		hash_map<size_t, size_t> h;
		int mycountis = 0;
		for (IntersectionsIterator iItr=isItr->begin();iItr!=isItr->end(); ++iItr, ++idx)
		{
			mycountis = mycountis + 1;
			if (h.find(iItr->getVertex()) == h.end() ||
				   iItr->getTime() < is[h[iItr->getVertex()]].getTime())
				h[iItr->getVertex()] = idx;
		...
		//std::cout<<"this iv has is of "<<mycountis<<std::endl;
		ivs.push_back(InterferenceVolume(this, *isItr, h));
		mycount = mycount+1;
	}
	//std::cout<<"total ivs: "<<mycount<<std::endl;
}
...
void Curve::groupIntersections(Intersections &inputInters, vector<Intersections> &intersections)
{
	...
	for (IntersectionsIterator iItr=inputInters.begin(); iItr!=inputInters.end(); ++iItr)
	{
		...
		for (size_t i=0; i<3; ++i)
		{
			bool bp = false;
			bool vp = false;
			int inv = verts[i]/m_Npv;
			int pi = 0;
			if(inv>=m_nv)
			{
				bp =true;
				inv = (verts[i]-m_nv*m_Npv)/m_Npb; 
				pi = (verts[i]-m_nv*m_Npv)%m_Npb;
			}
			else
			{
				pi = verts[i]%m_Npv;
			...
			// add verts[i]'s neighboring vertices(Nexten-1)
			for (size_t je=1;je<m_nexten;++je)
			{
				if(vp)
				{
				 	int inda = (pi - je)%m_Npv;
					s.insert(inv*m_Npv+inda);
					inda = (pi + je)%m_Npv;
					s.insert(inv*m_Npv+inda);
				}
				else if(bp)
				{
				 	...
				 	int inda = (pi - je)%m_Npb;
					s.insert(inv*m_Npb+inda + m_nv*m_Npv);
					inda = (pi + je)%m_Npb;
					s.insert(inv*m_Npb+inda + m_nv*m_Npv);
				}
				else
...
bool Curve::getCurveIntersections(Intersections &is)
{
	...
	for (size_t i=0; i<nbrEdges; ++i)
	{
		...
		for (size_t j=0; j<2; ++j)
		{
			...
			//xMin[j] = min(x10[j], min(x11[j], min(x20[j], x21[j]))) - getMinimumSeparation();
			//xMax[j] = max(x10[j], min(x11[j], min(x20[j], x21[j]))) + getMinimumSeparation();
		}
		xMin[2] = -getMinimumSeparation();
		xMax[2] =  getMinimumSeparation();
	...
	for (CandidatesIterator cItr=hits.begin(); cItr!=hits.end(); ++cItr)
	{
		...
		if(xInd[2*cItr->first] == xInd[2*e[2*cItr->second+0]])
			continue;
	...
	//std:cout<<"total is: "<<mycount_is<<endl;
	return !is.empty();
}
...
{
        /*
	std::cout<<"get intersection of p, a, b: "<<std::endl;
	...
	for (int i=0; i<cnt_total; ++i)
	{
		...
		if ( (std::abs(im_total[i]) < 1e-16) && (rl_total[i] >= -1e-16) && (rl_total[i] <= (1.0+1e-16)))
		{
			...
			//double d = (p - (a + ab * s)).norm() - getMinimumSeparation();
