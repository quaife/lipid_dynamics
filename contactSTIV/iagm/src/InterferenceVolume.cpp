	...
	for (hash_map<size_t, size_t>::iterator iItr=vertIndices.begin(); iItr!=vertIndices.end(); ++iItr)
	{
		...
		assert((1.0 - t)!=0);
                ...
                if(idx == 0)
                  ...
                  tmp = (1.0 - t) * ((1-i.getAlpha())*v[1] + i.getAlpha()*v[2]).dot(n);
		...
		Idm(1,1) = 1;
		  ...
		  Vector2d n_eps(e_eps[1], -e_eps[0]);
                ...
                */
                ...
                {
                  m_dVdq.segment<2>(2*i.getVertex())      += 
                    sign * (
                        // v[1] part
                        -dtdq[0] * (1 - i.getAlpha()) * v[1].dot(n) +
                        0 * n + 
                        (1 - i.getTime()) * (1 - i.getAlpha()) * dndq1.transpose() * v[1] +
                        -dwdq[0] * (1 - i.getTime()) * v[1].dot(n) +
                        // v[2] part
                        -dtdq[0] * i.getAlpha() * v[2].dot(n) +
                        0 * n +
                        (1 - i.getTime()) * i.getAlpha() * dndq1.transpose() * v[2] +
                        dwdq[0] * (1 - i.getTime()) * v[2].dot(n)
                    );

                  m_dVdq.segment<2>(2*i.getEdgeVertex1()) += 
                    sign * (
                        // v[1] part
                        -dtdq[1] * (1 - i.getAlpha()) * v[1].dot(n) + 
                        (1 - i.getTime()) * (1 - i.getAlpha()) * n +
                        (1 - i.getTime()) * (1 - i.getAlpha()) * dndq2.transpose() * v[1] +
                        -dwdq[1] * (1 - i.getTime()) * v[1].dot(n) +
                        // v[2] part
                        -dtdq[1] * i.getAlpha() * v[2].dot(n) +
                        0 * n +
                        (1 - i.getTime()) * i.getAlpha() * dndq2.transpose() * v[2] +
                        dwdq[1] * (1 - i.getTime()) * v[2].dot(n)
                    );
                  
                  m_dVdq.segment<2>(2*i.getEdgeVertex2()) += 
                    sign * (
                        // v[1] part
                        -dtdq[2] * (1 - i.getAlpha()) * v[1].dot(n) +
                        0 * n +
                        (1 - i.getTime()) * (1 - i.getAlpha()) * dndq3.transpose() * v[1] +
                        -dwdq[2] * (1 - i.getTime()) * v[1].dot(n) +
                        // v[2] part
                        -dtdq[2] * i.getAlpha() * v[2].dot(n) +
                        (1 - i.getTime()) * i.getAlpha() * n +
                        (1 - i.getTime()) * i.getAlpha() * dndq3.transpose() * v[2] +
                        dwdq[2] * (1 - i.getTime()) * v[2].dot(n)
                    );
...
void InterferenceVolume::getTimeGradient(Intersection &i, size_t idx, double *x0, double *x1, Vector2d &dtdq, Curve *C)
{
	...
	double a1 = x1x2.dot(v3v2) + v1v2.dot(x3x2);
        ...
        else if(i.getAlpha() < 1 && i.getAlpha() > 0 && C->getMinimumSeparation() != 0.0)
        {
          ...
          double scale = -1.0 / (4.0*(a2*a2)*t*t*t + 3.0*(2.0*a1*a2)*t*t + 
            2.0*(a1*a1 + 2.0*a0*a2 - h*h*v3v2.dot(v3v2))*t + (2.0*a0*a1 - h*h*2.0*x3x2.dot(v3v2)));
          
          Vector2d num;
          if(idx==0)
            num = (2.0*a2*v3v2)*t*t*t*t + (2.0*a1*v3v2 + 2.0*a2*x3x2)*t*t*t + 
              (2.0*a1*x3x2 + 2.0*a0*v3v2)*t*t + (2.0*a0*x3x2)*t;
          else if(idx==1)
            num = (2.0*a2*(-v3v2-TM*v1v2))*t*t*t*t + (2.0*a1*(-v3v2-TM*v1v2) + 2.0*a2*(-TM*x1x2-x3x2))*t*t*t +
              (2.0*a1*(-TM*x1x2 - x3x2) + 2.0*a0*(-v3v2 - TM*v1v2) + h*h*2.0*(v3-v2))*t*t + (2.0*a0*(-TM*x1x2-x3x2) + h*h*2.0*TM*x3x2)*t;
          else
            num = (2.0*a2*TM*v1v2)*t*t*t*t + (2.0*a1*(TM*v1v2) + 2.0*a2*(TM*x1x2))*t*t*t + 
              (2.0*a1*(TM*x1x2) + 2.0*a0*(TM*v1v2) - h*h*2.0*(v3-v2) )*t*t + (2.0*a0*(TM*x1x2) - h*h*2.0*TM*x3x2)*t;
